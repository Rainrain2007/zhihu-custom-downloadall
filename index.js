...
(() => {
  ...
  var printPeopleAnswer = async () => {
    ...
    nButton.onclick = async function() {
      ...
      const content = data.map((item) => {
        const created = new Date(item.created_time * 1e3).toLocaleString();
        const updated = new Date(item.updated_time * 1e3).toLocaleString();
    ...
    autoButton.onclick = async function() {
      ...
      while (me.getAttribute("data-running") === "true") {
        ...
        await new Promise((r) => setTimeout(r, 1e3));
        let nextBtn = document.querySelector(".PaginationButton-next") || document.querySelector("button.PaginationButton:last-child");
        if (nextBtn && !nextBtn.disabled && (nextBtn.innerText.includes("下一页") || nextBtn.querySelector("svg"))) {
          ...
          while (waitTime < MAX_WAIT && me.getAttribute("data-running") === "true") {
            ...
            if (waitTime === 10 || waitTime === 25) {
              ...
              const retryBtn = document.querySelector(".PaginationButton-next") || document.querySelector("button.PaginationButton:last-child");
  ...
  var printPeopleArticles = async () => {
    ...
    nButton.onclick = async function() {
      ...
      eventBtn.disabled = true;
      const content = data.map((item) => {
        const created = new Date(item.created * 1e3).toLocaleString();
        const updated = new Date(item.updated * 1e3).toLocaleString();
        const timeLine = `<div style="color: #888; font-size: 12px; margin-bottom: 10px; background: #f5f5f5; padding: 5px 10px; border-radius: 4px;">
        📅 发布于: ${created} ${updated !== created ? ` | ✏️ 最后编辑于: ${updated}` : ""}
      </div>`;
  ...
  var printPeoplePins = async () => {
    ...
    nButton.onclick = async function() {
      ...
      const content = data.map((item) => {
        ...
        const created = timeStamp ? new Date(timeStamp * 1e3).toLocaleString() : "未知时间";
        const timeLine = `<div style="color: #888; font-size: 12px; margin-bottom: 10px; background: #f5f5f5; padding: 5px 10px; border-radius: 4px;">
        📅 发布于: ${created}
      </div>`;
        let pinContent = "";
      ...
      if (HTML_HOOTS.includes(hostname) && !window.frameElement) {
        try {
          ...
          try {
            ...
            const articleTargets = Object.values(prevArticles);
            articleTargets.length && setUserArticle(articleTargets);
          } catch {
          }
          try {
            const prevPins = JsData.initialState.entities.pins;
            const pinTargets = Object.values(prevPins).filter((p) => p && (p.id || p.target?.id));
